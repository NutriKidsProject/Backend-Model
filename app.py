...
# Set path ke file kredensial
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/app/nutrikids-key.json"
...
def save_to_firestore(response_data):
    try:
        # Pilih koleksi dan auto-generate document ID
        doc_ref = db.collection("predictions").document()
        doc_ref.set(response_data)  # Simpan data ke Firestore
        print(f"Data saved to Firestore with ID: {doc_ref.id}")
        return True, f"Data saved to Firestore with ID: {doc_ref.id}"
    except Exception as e:
...
def predict():
    ...
    try:
        ...
        success, message = save_to_firestore(response_data)

        # Tambahkan status pada response
        response_data["firestore_status"] = message

        if success:
            return jsonify(response_data), 200
        else:
            return jsonify({"error": "Failed to save data to Firestore", "details": message}), 500
...
if __name__ == "__main__":
    # Port 8080 diperlukan untuk Cloud Run
